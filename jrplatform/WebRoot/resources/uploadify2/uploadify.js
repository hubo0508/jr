		...
		a.extend(a.fn, {
			uploadify : function(b) {
				a(this).each(function() {
					settings = a.extend({
								id : a(this).attr("id"),
								uploader : 'resources/uploadify2/uploadify.swf',
					...
					function(j, h, i) 
					{
						if (j.data.action(j, h, i) !== false) 
						{
							...
							+'  <div class="cancel" style="margin-top:-27px; margin-left:78px;">'
							...
							+'  <span class="percentage" style="font-weight:bold; color:#339E35"></span>'
					...
					a(this).bind("uploadifyQueueFull", {
								...
								if (h.data.action(h, i) !== false) {
									$.MessageBox({message:"The queue is full.  The max size is "
											+ i + "."});
								}
							});
					...
					var c = [];
					a(this).bind("uploadifyError", {action : settings.onError}, 
					function(l, h, k, j) 
					{
					...
					a(this).bind(
						"uploadifyComplete", {action : settings.onComplete}, 
						function(k, h, j, i, l) 
						{
							if (k.data.action(k, h, j, unescape(i), l) !== false) 
							{
								a("#" + a(this).attr("id") + h + " .percentage").text(" - 上传完成");
								//a("#" + a(this).attr("id") + h).fadeOut(250, function() {a(this).remove();alert('xx');});
							}
						});
