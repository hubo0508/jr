...
<body sroll="no">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="450" class="add_edit_table" id="table">
    	<tr>
            ...
            <td align="left"><input id="major_category" type="text" class="txt" style="width:100%" value="${p.major_category }"/></td>
        </tr>
        <tr>
        	<td width="98" align="right">描述</td>
            <td align="left"><textarea id="major_category_desc" class="txt" style="width:100%; height:80px;">${p.major_category_desc }</textarea></td>
        </tr>
    </table>
...
<script type="text/javascript">
	$(function(){
		PopupWindowScrollbar("majorAdd_Edit",".scrollbar");
	...
	function saveHandler(){
		var _major_category      = $("#major_category").attr("value");
		var _major_category_desc = $("#major_category_desc").val();
		if(!check(_major_category,_major_category_desc)){
			return ;
		}
		...
		$.ajax({
			type : "post",
			url : "${pageContext.servletContext.contextPath}/majorCategory/save",
			dataType:"json",
			data:"major_category="+_major_category+"&major_category_desc="+_major_category_desc+"&id=${p.id }",
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			success : function(msg) {
				if((msg.success+"") == "true"){
					$("#loading").css("display","block").css("background-color","#090").html("保存成功！");
					closeHandler();
					window.parent.loading("${pageContext.servletContext.contextPath}/majorCategory/majorJump");
				}else{
	...
	function closeHandler(){window.parent.remove("majorAdd_Edit");}
</script>
</html>
