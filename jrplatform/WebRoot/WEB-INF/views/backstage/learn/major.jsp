...
<body sroll="no">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="title_t">
    	<tr>
        	<td>
            	<div class="labelDiv">    	
                    <div class="labelTitle">维护类别管理</div>
...
<script type="text/javascript">
	...
	function setSearchresult(msg){
		...
		$.each(msg.object,function(idx,item){
			var _id               = item.id;
			var _major_category      = item.major_category       || '&nbsp';
			var _major_category_desc = item.major_category_desc  || '&nbsp';
			...
			_table += 
	            ...
	            '    <td class="c">'+_major_category+'</td>'+
	            '    <td class="l">'+_major_category_desc+'</td>'+
	            '</tr>';
	...
	function clickButtonCallback(text){
		...
		if(text == "新增"){
			window.parent.popup({ 
				message: 'popup_links', 
				content: "${pageContext.servletContext.contextPath}/majorCategory/addJump",
				title:"新增维护类别",
	...
	function deleteByIds(ids){
		...
		$.ajax( {
			type : "post",
			url : "${pageContext.servletContext.contextPath}/majorCategory/delete",
			dataType:"json",
			data:"ids="+ids,
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			success : function(msg) {
	...
	function edit(id){
		window.parent.popup({ 
			message: 'popup_links', 
			content: "${pageContext.servletContext.contextPath}/majorCategory/editJump?id="+id,
			title:"编辑维护类别",
			css: {width: '500px',height:'300px'},
			pw_id :"majorAdd_Edit",
		});	
