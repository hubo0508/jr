...
<body sroll="no">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="title_t" height="100%">
        ...
        <tr>
        	<td valign="top">
            	<!-- <textarea class="ckeditor" name="range">${p.ranges }</textarea> -->
            	<script id="editor" type="text/plain" style="width:100%;height:100%;">${p.ranges }</script>
            </td>
            <td widht="20"></td>
        </tr>
    </table>
...
<script type="text/javascript">
	...
	function clickButtonCallback(text){
		if(text == "刷新/查询")
		{
			window.parent.loading("${pageContext.servletContext.contextPath}/service/rangeJump");
			return;
		}
	...
	function saveHandler(){
		...
		var _range = encodeURIComponent(UE.getEditor('editor').getContent());
		if(_range.length > 65535){alert("描述长度应小于65535字符！");return false;}
		
		$.ajax({
			type : "post",
			url : "${pageContext.servletContext.contextPath}/service/saveRange",
			dataType:"json",
			data:"id=${p.id}&range="+_range,
