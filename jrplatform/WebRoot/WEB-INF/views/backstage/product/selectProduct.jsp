...
<body sroll="no">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="title_t">
    	<tr>
        	<td>
            	<div class="labelDiv">    	
                    <div class="g_gray_line workTitleLine" style="margin-top:40px; margin-right:20px;"></div>
                    
                    <div class="linksButton">
                       <div class="btn"><span class="hline"></span><div class="c">
                       		<span class="icon excel">&nbsp;</span><span class="text">确认</span>
                       </div></div>
                       <div class="btn"><span class="hline"></span><div class="c">
    ...
    <div class="pc_win_nav" style="font-size:12px;">
        <div class="g_green_line"></div>
        <div class="bottom_navigation">
            <div id="close" onclick="closeHandler()">关闭</div>
            <div id="save" onclick="clickButtonCallback('确认')">保存</div>
        </div>
        <div class="g_green_line"></div>
    </div>
</body>
<script type="text/javascript">
	...
	$(function(){	
		...
		//PopupWindowScrollbar("selectProduct",".scrollbar");
	});
	...
	function setSearchresult(msg)
	{
		...
		$.each(msg.object,function(idx,item){ 
			...
			_table += '<tr>'+
					...
					'	<td class="c"><input type="checkbox" name="tdcb" value="'+_id+'"/></td>'+
	           		'   <td class="c"><span class="tablebtn" onclick="confirm('+_id+')">确认</span></td>'+
		...
		$.cloneTableHead();		
		$.tableAutoScroll({fixed_target:[".title_t:height",55]});
	}
		...
		if(text == "确认")
		{
			var ids = "";
			 ...
			 window.parent.popup({ 
				...
				content: "确认选择数据设置滚动显示!",
				callback:function(type){
					if(type == "confirm"){_confirm(ids)}
					return true;
				},
				css: {width: '250'} 
			 });
	...
	function _confirm(ids){
		ids = ids.substring(0,ids.length-1);
		
		$("#loading").css("display","block").css("background-color","#090").html("正在保存数据，请稍后！");
		
		$.ajax( {
			type : "post",
			url : "${pageContext.servletContext.contextPath}/product/confirmScroll",
			dataType:"json",
			data:"ids="+ids,
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			success : function(msg) {
				if((msg.success+"") == "true"){
					...
					closeHandler();
					window.parent.loading("${pageContext.servletContext.contextPath}/product/productScrollListJump");
				}else{
					$("#loading").fadeOut();
					window.parent.alert(msg.message);	
				}
	...
	function confirm(id){
		window.parent.popup({ 
			...
			content: "确认选择数据设置滚动显示!",
			callback:function(type){
				if(type == "confirm"){_confirm(id+",")} return true;
			},
			css: {width: '250'} 
		 });
	...
	function closeHandler(){window.parent.remove("selectProduct");}
