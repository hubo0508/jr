...
<body sroll="no">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="title_t">
    	<tr>
        	<td>
            	<div class="labelDiv">    	
                    <div class="labelTitle">金嵘发展历程</div>
...
<script type="text/javascript">
	...
	function getList(pageSize,start)
	{
		...
		$.ajax( {
			type : "post",
			url : "${pageContext.servletContext.contextPath}/growingProcess/list",
			dataType:"json",
	...
	function setSearchresult(msg){
		...
		$.each(msg.object,function(idx,item){
			var _id               = item.id;
			var _stage_date       = item.year_date+"-"+item.month_date;
			var _stage_desc  = item.stage_desc  || '&nbsp';
			...
			_table += 
	            ...
	            '    <td class="c">'+_stage_date+'</td>'+
	            '    <td class="l">'+_stage_desc+'</td>'+
	            '</tr>';
	...
	function clickButtonCallback(text){
		...
		if(text == "新增"){
			window.parent.popup({ 
				message: 'popup_links', 
				content: "${pageContext.servletContext.contextPath}/growingProcess/addJump",
				title:"新增发展历程",
				css: {width: '500px',height:'300px'},
				pw_id :"growingProcess_add_edit"
			});	
		}
	...
	function deleteByIds(ids){
		...
		$.ajax( {
			type : "post",
			url : "${pageContext.servletContext.contextPath}/growingProcess/delete",
			dataType:"json",
			data:"ids="+ids,
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			success : function(msg) {
	...
	function edit(id){
		window.parent.popup({ 
			message: 'popup_links', 
			content: "${pageContext.servletContext.contextPath}/growingProcess/editJump?id="+id,
			title:"编辑发展历程",
			css: {width: '500px',height:'300px'},
			pw_id :"growingProcess_add_edit",
		});	
