...
<body sroll="no">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="title_t" height="100%">
        ...
        <tr>
        	<td valign="top">
            	<script id="editor" type="text/plain" >${p.protocold }</script>
            </td>
            <td widht="20"></td>
        </tr>
    </table>
...
<script type="text/javascript">
	...
	function clickButtonCallback(text){
		if(text == "刷新/查询")
		{
			window.parent.loading("${pageContext.servletContext.contextPath}/aboutdesc/protocoldescJump");
			return;
		}
	...
	function saveHandler(){
		...
		var _protocold = encodeURIComponent(UE.getEditor('editor').getContent());
		if(_protocold.length > 65535){alert("描述长度应小于65535字符！");return false;}
		
		$.ajax({
			type : "post",
			url : "${pageContext.servletContext.contextPath}/aboutdesc/saveToProtocold",
			dataType:"json",
			data:"id=${p.id}&protocold="+_protocold,
