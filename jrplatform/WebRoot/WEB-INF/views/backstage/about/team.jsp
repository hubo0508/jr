        ...
        <table class="table table-bordered" width="100%" style="padding-right:20px; table-layout: fixed; z-index:0">
           <tbody id="tbody">
           		<tr>
		            ...
		            <th width="80" class="c">排序</th>
		            <th width="80" class="c">姓名</th>
		            <th width="150" class="l">职位简介</th>
		            <th class="l">详细描述</th>
	            </tr>
           </tbody>            
        </table>
...
<script type="text/javascript">
	...
	function getList(pageSize,start)
	{
		...
		$.ajax( {
			type : "post",
			url : "${pageContext.servletContext.contextPath}/team/list",
			dataType:"json",
	...
	function setSearchresult(msg){
		...
		$.each(msg.object,function(idx,item){
			...
			_table += 
	            ...
	            '    <td class="c">'+item.custom_sort+'</td>'+
	            '    <td class="c">'+item.name+'</td>';
	            
	            if(_url == ""){
	            	_table +=  '<td class="c">&nbsp;</td>';
	            }else{
	            	_table +=  '<td class="c"><a class="zp"  target="_blank" href="'+item.url+'">预览</a></td>';
	            }
	           
	           _table +=
	            '    <td class="l">'+item.job_desc+'</td>'+
	            '    <td class="l">'+_job_info+'</td>'+
	            '</tr>';
		...
		var _head = '<tr>'+
			        ...
			        '    <th width="30" height="30" class="c">选择</th>'+
			        '    <th width="65" class="c">编辑</th>'+
			        '    <th width="30" class="c">排序</th>'+
			        '    <th width="70" class="c">姓名</th>'+
			        '    <th width="70" class="c">照片</th>'+
			        '    <th width="200" class="l">职位简介</th>'+
			        '    <th class="l">详细描述</th>'+
		            '</tr>';
	...
	function clickButtonCallback(text){
		...
		if(text == "新增"){
			window.parent.popup({ 
				message: 'popup_links', 
				content: "${pageContext.servletContext.contextPath}/team/addJump",
				title:"新增成员",
				css: {width: '600px',height:'80%'},
				pw_id :"team_add_edit"
			});	
		}
	...
	function deleteByIds(ids){
		...
		$.ajax( {
			type : "post",
			url : "${pageContext.servletContext.contextPath}/team/delete",
			dataType:"json",
			data:"ids="+ids,
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			success : function(msg) {
	...
	function edit(id){
		window.parent.popup({ 
			message: 'popup_links', 
			content: "${pageContext.servletContext.contextPath}/team/editJump?id="+id,
			title:"编辑成员数据",
			css: {width: '600px',height:'80%'},
			pw_id :"team_add_edit",
		});	
