...
<body sroll="no">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="title_t">
    	<tr>
        	<td>
            	<div class="labelDiv">    	
                    <div class="labelTitle">客户目录管理</div>
        ...
        <table class="table table-bordered" width="100%" style="padding-right:20px; table-layout: fixed; z-index:0">
           <tbody id="tbody">
           		<tr>
		            ...
		            <th width="200" class="c">客户名称</th>
		            <th width="80" class="c">LOG图片</th>
		            <th class="l">详细描述</th>
	            </tr>
           </tbody>            
        </table>
...
<script type="text/javascript">
	...
	function getList(pageSize,start)
	{
		...
		$.ajax( {
			type : "post",
			url : "${pageContext.servletContext.contextPath}/customerController/list",
			dataType:"json",
	...
	function setSearchresult(msg){
		...
		$.each(msg.object,function(idx,item){
			...
			_table += 
	            ...
	            '    <td class="c">'+item.customer_name+'</td>';
	            
	            if(item.image_url == ""){
	            	_table +=  '<td class="c">&nbsp;</td>';
	            }else{
	            	_table +=  '<td class="c"><a class="zp"  target="_blank" href="'+item.image_url+'">预览</a></td>';
	            }
	           
	           var _desc = item.description || "&nbsp;"
	           _table +='<td class="l">'+_desc.replace(/<[^>]+>/g,"")+'</td></tr>';
	...
	function clickButtonCallback(text){
		...
		if(text == "新增"){
			window.parent.popup({ 
				message: 'popup_links', 
				content: "${pageContext.servletContext.contextPath}/customerController/addJump",
				title:"新增数据",
				css: {width: '800px',height:'80%'},
				pw_id :"customer_add_edit"
			});	
		}
	...
	function deleteByIds(ids){
		...
		$.ajax( {
			type : "post",
			url : "${pageContext.servletContext.contextPath}/customerController/delete",
			dataType:"json",
			data:"ids="+ids,
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			success : function(msg) {
	...
	function edit(id){
		window.parent.popup({ 
			message: 'popup_links', 
			content: "${pageContext.servletContext.contextPath}/customerController/editJump?id="+id,
			title:"编辑数据",
			css: {width: '800px',height:'80%'},
			pw_id :"customer_add_edit",
		});	
