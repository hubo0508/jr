...
<body sroll="no">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="title_t">
    	<tr>
        	<td>
            	<div class="labelDiv">    	
                    <div class="labelTitle">配件专业类别管理</div>
...
<script type="text/javascript">
	...
	function setSearchresult(msg){
		...
		$.each(msg.object,function(idx,item){
			var _id               = item.id;
			var _fitting_category      = item.fitting_category       || '&nbsp';
			var _fitting_category_desc = item.fitting_category_desc  || '&nbsp';
			...
			_table += 
	            ...
	            '    <td class="c">'+_fitting_category+'</td>'+
	            '    <td class="l">'+_fitting_category_desc+'</td>'+
	            '</tr>';
	...
	function clickButtonCallback(text){
		...
		if(text == "新增"){
			window.parent.popup({ 
				message: 'popup_links', 
				content: "${pageContext.servletContext.contextPath}/fittingSpecialtyCate/addJump",
				title:"新增配件专业类别",
				css: {width: '500px',height:'300px'},
				pw_id :"specialtyCategory_add_edit"
			});	
		}	
	...
	function deleteByIds(ids){
		...
		$.ajax( {
			type : "post",
			url : "${pageContext.servletContext.contextPath}/fittingSpecialtyCate/deleteByIds",
			dataType:"json",
			data:"ids="+ids,
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			success : function(msg) {
	...
	function edit(id){
		window.parent.popup({ 
			message: 'popup_links', 
			content: "${pageContext.servletContext.contextPath}/fittingSpecialtyCate/editJump?id="+id,
			title:"编辑配件专业分类",
			css: {width: '500px',height:'300px'},
			pw_id :"specialtyCategory_add_edit",
		});	
